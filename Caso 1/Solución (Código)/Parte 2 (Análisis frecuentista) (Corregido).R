...
# Tamaños de remuestra para bootstrap.
n1 = length(dataILEC)
n2 = length(dataCLEC)
...
for (i in 1:mBoots){
  GenDataCLEC <- sample(dataCLEC, size = n2,replace = T) # Remuestreo CLEC
  GenDataILEC <- sample(dataILEC, size = n1,replace = T) # Remuestreo ILEC
...
set.seed(20122002)
for (i in 1:B){
  remuestraCLEC = rexp(n2, rate = lambda1)
  remuestraILEC = rexp(n1, rate = lambda2)
...
hist(MediaEta,col = 'gray90',border = 'gray90',main = 'Análisis frecuentista',prob = T,
     ylab = 'Densidad',ylim = c(0,0.12),xlab = TeX('$\\eta = \\lambda_1 - \\lambda_2$'))
